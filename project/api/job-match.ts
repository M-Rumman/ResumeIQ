...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { resumeText = '', location = '', title = '', workPreference = 'both' } = req.body || {}; if (String(resumeText).trim().length < 50) return res.status(400).json({ error: 'Resume text is too short.' });
