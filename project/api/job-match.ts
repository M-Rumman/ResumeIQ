...
import { localProfile, searchJobs } from './_lib/jobMatch.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try { const profile = localProfile(String(resumeText)); const query = String(title || profile.jobTitles[0] || profile.technicalSkills.slice(0, 3).join(' ') || 'graduate'); const found = await searchJobs(query, String(location)); if (!found.jobs.length) return res.status(200).json({ profile, jobs: [], sources: found.sources }); const ranked = await understandAndRank(String(resumeText), profile, found.jobs); return res.status(200).json({ ...ranked, sources: found.sources }); } catch (error) { console.error('[job-match]', { type: error instanceof Error ? error.name : 'unknown' }); return res.status(502).json({ error: 'Job matching is temporarily unavailable. Please try again shortly.' }); }
}
