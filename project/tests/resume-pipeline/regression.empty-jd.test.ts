...
const emptyJD = `  `; // Just whitespace

async function test() {
  console.log('Running analysis with empty JD...');
  const engineResult = await runAnalysisPipeline({
    resumeText: priyaResume,
    jobDescriptionText: emptyJD,
    includePremium: true
  });
  console.log('EngineResult structure keys:', Object.keys(engineResult));
  
  const result = engineResult.legacyReport;
  if (!result) {
    console.error('legacyReport is undefined!');
    process.exit(1);
  }
  
  console.log('legacyReport structure keys:', Object.keys(result));
