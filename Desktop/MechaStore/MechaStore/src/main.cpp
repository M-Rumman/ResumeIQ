...
void customerFlow(Customer *cust) {
    ...
    while (running) {
        ...
        if (op == "1") browseComponents();
        else if (op == "2") addComponentByUser(cust);
        else if (op == "3") {
            browseComponents();
            ...
            cust->viewCart();
			cout << "Total: PKR" << cust->getCart().getCartTotal() << "\n";
        }
    ...
    while (running) {
        ...
        if (op == "1") browseComponents();
            ...
            try {
                ...
                cout << "New price (blank = keep): ";
                getline(cin, newPrice);

                cout << "New qty (blank = keep): ";
                getline(cin, newQty);

                admin->updateComponent(
                    *found,
